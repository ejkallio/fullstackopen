...
describe('Note app', () => {
  ...
  test('login fails with wrong password', async ({ page }) => {
    ...
    await expect(page.getByText('wrond credentials')).toBeVisible()

    const errorDiv = await page.locator('.error')
    await expect(errorDiv).toContainText('wrond credentials')
    await expect(errorDiv).toHaveCSS('border-style', 'solid')
    await expect(errorDiv).toHaveCSS('color', 'rgb(255, 0, 0)')

    await expect(page.getByText('Elias Kallio logged in')).not.toBeVisible()
  })
